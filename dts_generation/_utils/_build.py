...
from dts_generation._utils._shared import CACHE_PATH, COMPARISON_SCRIPTS_PATH, DATA_PATH, TEMPLATE_PATH, PackageDataMissingError, PackageInstallationError, ReproductionError, INSTALLATION_TIMEOUT

BUILD_SUB_PATH = Path("builds")
RUN_TIME_ANALYZER = "run-time-information-analyzer"
RUN_TIME_ANALYZER_PATH = BUILD_SUB_PATH / RUN_TIME_ANALYZER
DECLARATION_GENERATOR = "ts-declaration-file-generator"
DECLARATION_GENERATOR_PATH = BUILD_SUB_PATH / DECLARATION_GENERATOR
...
def build_npm_tools(build_path: Path, verbose_setup: bool) -> None:
    with printer(f"Building npm tools:"):
        ...
        if not dir_empty(output_path) and (output_path / "transpile.js").is_file():
            ...
            return None
        create_dir(output_path, overwrite=True)
        create_file(output_path / "package.json", COMPARISON_SCRIPTS_PATH / "package.json")
        create_file(output_path / "package-lock.json", COMPARISON_SCRIPTS_PATH / "package-lock.json")
        create_file(output_path / "transpile.js", COMPARISON_SCRIPTS_PATH / "transpile.js")
        shell(
...
def get_package_json(generation_path: Path) -> Optional[str]:
    try:
...
def get_readme(generation_path: Path) -> Optional[str]:
    try:
...
def get_main(generation_path: Path) -> Optional[str]:
    try:
        repository_path = generation_path / REPOSITORY_PATH
...
def get_tests(generation_path: Path) -> list[tuple[str, str]]:
    try:
        tests = {}
        repository_path = generation_path / REPOSITORY_PATH
